...
static mcu_status_t exchange(mcuco_t *mcu, const uint8_t *frame, ssize_t frame_len,
                                 ...
                                 action_t action, port_t out_port, uint8_t out_pin)
{
    ...
    uint8_t frame[PROTOCOL_MAX_COMMAND_FRAME];
    ssize_t frame_len = protocol_gpio_irq_bind(edge, in_port, in_pin, action, out_port,
                                               out_pin, frame, sizeof(frame));
